...
simple_stomp_init(simple_stomp_t *ctx,
    const simple_string_t *host, int port, const simple_string_t *client_id, error_handler eh)
{
    Frame frame;
    ...
    size_t nheaders = client_id != NULL ? 2 : 0;
    ...
    makeframe(&frame, MAKE_STRING("CONNECT"), headers, nheaders, NULL);
